...
<tr><td><img src="packages/vvvvvv-mmiyoo/assets/icon.png" alt="VVVVVV icon" width="32" height="32">&nbsp;<code>vvvvvv-mmiyoo</code></td><td>VVVVVV 2.3.6, built from upstream source against the shared <code>sdl2-mmiyoo-lib</code>/<code>sdl2-mmiyoo-addons</code> providers via the Union toolchain container.</td><td>yes</td><td>no</td><td>Port artifact type; builds with <code>sdl2_gles: no</code> (no GL/EGL/GLES symbols) and ships only <code>libSDL2_mixer</code> from the addons bundle. Retail <code>data.zip</code> is proprietary and not bundled. See <code>packages/vvvvvv-mmiyoo/README.md</code>. <code>build_all: false</code>.</td></tr>
<tr><td><img src="packages/dungeonrush-mmiyoo/assets/icon.png" alt="DungeonRush icon" width="32" height="32">&nbsp;<code>dungeonrush-mmiyoo</code></td><td>DungeonRush, built from a pinned fork commit against the shared <code>sdl2-mmiyoo-lib</code>/<code>sdl2-mmiyoo-addons</code> providers via the Union toolchain container.</td><td>yes</td><td>yes</td><td>Port artifact type. Known rendering/scaling caveats: upstream is native 1440x960 against a 640x480 panel — the <code>.port</code> script requests a non-uniform stretch to fill the panel rather than letterbox, and a build-time patch bumps the HUD/menu font size for legibility after that downscale; sprites still read small since the game still renders at 1440x960 internally. A title-screen sprite sheet and two BGM tracks needed build-time asset fixes (oversized texture past the renderer's 4096px cap; a multiplexed video stream the cross-built audio decoder can't demux). All three fixes are kept as build-time transforms/patches in the package rather than as permanent commits on the pinned fork, to avoid maintaining an ever-diverging source copy. See <code>packages/dungeonrush-mmiyoo/README.md</code>. <code>build_all: true</code>.</td></tr>
</tbody>
</table>
...
<tr><td><code>sdl2-mmiyoo-addons</code></td><td>SDL2_image, SDL2_ttf, SDL2_mixer, and SDL2_net, built via <code>scripts/mksdl2.sh</code> in <code>SDL2_SKIP_CORE=1</code> mode against the <code>sdl2-mmiyoo-lib</code> provider.</td><td>—</td><td>Not invoked directly — built on demand via <code>depends_on</code> whenever a consumer package needs it.</td><td>Not a standalone deliverable; consumed only as <code>MMIYOO_SDL2_ADDONS_PREFIX</code> by dependent packages' <code>build.sh</code>. <code>build_all: false</code>.</td></tr>
</tbody>
</table>
