...
tcpdump_image="${MIYOO_TCPDUMP_IMAGE:-${docker_image}-tcpdump-build-v2}"
make_jobs="${MAKE_JOBS:-}"
...
ensure_tcpdump_image() {
  ensure_toolchain_image
  if docker image inspect "$tcpdump_image" >/dev/null 2>&1; then
    return
  fi
  log "Building tcpdump dependency image $tcpdump_image"
  docker build \
    --build-arg "BASE_IMAGE=$docker_image" \
    -t "$tcpdump_image" \
    "$repo_root/packages/tcpdump-mmiyoo"
}
