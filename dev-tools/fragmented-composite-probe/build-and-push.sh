...
# inline). Cross-compiles dev-tools/fragmented-composite-probe/probe.c against an
# already-built sdl2-mmiyoo-lib bundle and pushes it to the device.
#
# Usage:
#   scripts/build-package.sh sdl2-mmiyoo-lib   # build the driver bundle first
#   dev-tools/fragmented-composite-probe/build-and-push.sh
set -euo pipefail
...
stage_dir="$repo_root/work/fragmented-composite-probe/stage"
device_ip="${MMIYOO_DEVICE_IP:-192.168.1.78}"
device_user="${MMIYOO_DEVICE_USER:-onion}"
device_pass="${MMIYOO_DEVICE_PASS:-onion}"
device_dir="${MMIYOO_DEVICE_APP_DIR:-/mnt/SDCARD/App/FragmentedCompositeProbe}"

log() { printf '[fragmented-composite-probe] %s\n' "$*"; }
...
log "Run on-device with: cd $device_dir && LD_LIBRARY_PATH=./lib:\$LD_LIBRARY_PATH ./probe [small_count]"
log "  small_count (default 200): how many small textures to create before the oversized composite"
