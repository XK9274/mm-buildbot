/* Compares three ways to alpha-blend a solid-color rect into an ARGB8888
 * surface: a 1x1 uncached scratch pixel stretched via MI_GFX_BitBlit
 * (SRCALPHA/INVSRCALPHA + ALPHACHANNEL, a plain alpha blend), a scalar CPU
 * "over" compositing loop, and its NEON counterpart. No SDL2 -- raw
 * MI_SYS/MI_GFX only; QuickFill has no blend parameters, so it isn't a
 * valid baseline here. */
#include <mi_sys.h>
#include <mi_gfx.h>
#include <neon.h>
...
/* HW_STRICT dispatches one MI_GFX_BitBlit and blocks on its fence before the
 * next -- a full hardware round-trip per call. HW_BATCHED dispatches
 * HW_BATCH blits back-to-back with no wait between them (matching the real
 * driver's GFX_AddTextureFence/deferred-flush pattern) and blocks once for
 * the whole batch, reporting the amortized per-op cost. Both are reported so
 * neither number is presented as the only truth. */
...
/* Fixed partial alpha, a plausible AA-edge midpoint; RGB varies per frame so
 * repeated runs aren't blending an already-saturated buffer into itself. */
#define BLEND_SRC_ALPHA 180u

typedef struct { int w, h; } RectSize;
static const RectSize g_sizes[] = {
    { 10, 1 }, { 50, 2 }, { 100, 3 }, { 197, 1 }, { 640, 1 },
    { 100, 20 }, { 100, 50 }, { 100, 100 }, { 140, 140 }, { 200, 200 }, { 300, 200 },
    { 320, 240 }, { 640, 480 },
};
...
static void ck(const char *fmt, ...)
{
    ...
    fprintf(stderr, "[blend-fill-bench] %s\n", line);
}
...
static int alloc_buf(Buf *b, MI_U32 size, MI_BOOL cached)
{
    ...
    if (MI_SYS_Mmap(b->phy, size, &b->vir, cached) != MI_SUCCESS) {
...
/* Plain alpha-blend factor setup: SRCALPHA/INVSRCALPHA, ALPHACHANNEL flag,
 * no colorkey or color/alpha modulation. */
static void build_blend_opt(MI_GFX_Opt_t *opt, const MI_GFX_Rect_t *clip)
{
...
/* This panel is mounted upside down. */
...
int main(int argc, char *argv[])
{
    ...
    ck("start, frames_per_variant=%ld font=%s src_alpha=%u", frames_per_variant, font_path, BLEND_SRC_ALPHA);
    ...
    /* Target surface for all three variants: a cached panel-sized buffer, so
     * the C/NEON variants pay the same kind of dst read/write cost the real
     * framebuffer target does. Flushed to screen (and re-primed) between
     * variants only, never inside a timed loop. */
    Buf surf = {0};
    if (alloc_buf(&surf, (MI_U32)(PANEL_W * PANEL_H * 4), TRUE) != 0) {
    ...
    /* 1x1 ARGB8888 scratch source for the hw variants, mapped uncached --
     * matches MMIYOO_FillViaBlit exactly (fill_scratch_vir is uncached so
     * the per-blit color write needs no flush). scratch_pitch=16 keeps
     * MI_GFX's 16-byte source-stride alignment check happy. */
    const int scratch_pitch = 16;
    Buf scratch = {0};
    if (alloc_buf(&scratch, (MI_U32)scratch_pitch, FALSE) != 0) {
        ck("scratch alloc failed, aborting");
        free_buf(&surf);
        return 1;
    }
    ...
    for (int si = 0; si < SIZE_COUNT; si++) {
        ...
        for (int vi = 0; vi < VARIANT_COUNT; vi++) {
            ...
            /* Reprime the dest rect to a known non-saturated color before
             * each variant so all three blend into the same starting state. */
            {
                MI_GFX_Rect_t prime = { 0, 0, (MI_U32)w, (MI_U32)h };
                MI_GFX_Opt_t prime_opt;
                MI_U16 prime_fence;
                build_plain_opt(&prime_opt, &prime, E_MI_GFX_ROTATE_0);
                if (MI_GFX_QuickFill(&gfx_surf, &prime, 0xFF204060u, &prime_fence) == MI_SUCCESS) {
                    MI_GFX_WaitAllDone(FALSE, prime_fence);
                }
            }

            if (variant == VARIANT_HW_BATCHED) {
                long batches = frames_per_variant / HW_BATCH;
                MI_GFX_Rect_t srcrect = { 0, 0, 1, 1 };
                MI_GFX_Rect_t dstrect = { 0, 0, (MI_U32)w, (MI_U32)h };
                MI_GFX_Surface_t scratch_surf;
                memset(&scratch_surf, 0, sizeof(scratch_surf));
                scratch_surf.phyAddr = scratch.phy;
                scratch_surf.eColorFmt = E_MI_GFX_FMT_ARGB8888;
                scratch_surf.u32Width = 1;
                scratch_surf.u32Height = 1;
                scratch_surf.u32Stride = (MI_U32)scratch_pitch;
                MI_GFX_Opt_t opt;
                build_blend_opt(&opt, &dstrect);
                ...
                for (long batch = 0; batch < batches; batch++) {
                    ...
                    for (int b = 0; b < HW_BATCH; b++) {
                        uint32_t color = (BLEND_SRC_ALPHA << 24) | (uint32_t)(((batch * HW_BATCH + b) * 61) & 0xFFFFFF);
                        *(uint32_t *)scratch.vir = color;
                        result = MI_GFX_BitBlit(&scratch_surf, &srcrect, &gfx_surf, &dstrect, &opt, &fences[b]);
                        if (result != MI_SUCCESS) break;
                    }
                    MI_GFX_WaitAllDone(TRUE, 0);
                    double t1 = now_us();
                    if (result != MI_SUCCESS) {
                        ck("%s/%s batch=%ld MI_GFX_BitBlit FAILED result=0x%x",
            ...
            } else if (variant == VARIANT_HW_STRICT) {
                MI_GFX_Rect_t srcrect = { 0, 0, 1, 1 };
                MI_GFX_Rect_t dstrect = { 0, 0, (MI_U32)w, (MI_U32)h };
                MI_GFX_Surface_t scratch_surf;
                memset(&scratch_surf, 0, sizeof(scratch_surf));
                ...
                for (long frame = 0; frame < frames_per_variant; frame++) {
                    uint32_t color = (BLEND_SRC_ALPHA << 24) | (uint32_t)((frame * 61) & 0xFFFFFF);

                    double t0 = now_us();
                    *(uint32_t *)scratch.vir = color;
                    MI_U16 fence;
                    MI_S32 result = MI_GFX_BitBlit(&scratch_surf, &srcrect, &gfx_surf, &dstrect, &opt, &fence);
                    if (result != MI_SUCCESS) {
                        ck("%s/%s frame=%ld MI_GFX_BitBlit FAILED result=0x%x",
                ...
                for (long frame = 0; frame < frames_per_variant; frame++) {
                    ...
                    if (variant == VARIANT_C) {
                        blend_solid_c32(surf.vir, color, (uint32_t)w, (uint32_t)h, (uint32_t)(PANEL_W * 4));
                    } else {
                        blend_solid_n32(surf.vir, color, (uint32_t)w, (uint32_t)h, (uint32_t)(PANEL_W * 4));
                    }
                    double t1 = now_us();
                    update_stats(st, t1 - t0);
                }
            ...
            if (fb_fd >= 0 && fb_surf.phyAddr) {
                ...
                MI_U16 band_fence;
                snprintf(banner, sizeof(banner), "blend %s  %s  avg=%.1fus  rot180",
