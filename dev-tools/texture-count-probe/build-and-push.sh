...
# inline). Cross-compiles dev-tools/texture-count-probe/probe.c against an
...
#   dev-tools/texture-count-probe/build-and-push.sh
set -euo pipefail
...
stage_dir="$repo_root/work/texture-count-probe/stage"
device_ip="${MMIYOO_DEVICE_IP:-192.168.1.78}"
device_user="${MMIYOO_DEVICE_USER:-onion}"
device_pass="${MMIYOO_DEVICE_PASS:-onion}"
device_dir="${MMIYOO_DEVICE_APP_DIR:-/mnt/SDCARD/App/TextureCountProbe}"

log() { printf '[texture-count-probe] %s\n' "$*"; }
...
log "Run on-device with: cd $device_dir && LD_LIBRARY_PATH=./lib:\$LD_LIBRARY_PATH ./probe [0|1]"
log "  arg 1 (default): keep every texture alive, like BlobbyVolley2 does"
log "  arg 0: destroy each texture before creating the next"
