#!/usr/bin/env bash
# Compiles probe.c into $1/fill-bench-probe (+ its on-screen-text font under
# $1/res). No push -- see dev-tools/probes-app/. Needs a TTF/OTF font
...
docker run --rm --user "$(id -u):$(id -g)" -e HOME=/tmp \
  ...
  -o /workspace/out/fill-bench-probe

install -m 644 "$font_path" "$out_dir/res/font.otf"

printf '%s\n' "$out_dir/fill-bench-probe"
